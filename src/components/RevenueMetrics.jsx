...
const RevenueMetrics = () => {
  ...
  const clickRevenue = ads.reduce((sum, ad) => sum + ad.clicks * ad.clickPrice, 0);
  const viewRevenue = ads.reduce((sum, ad) => sum + ad.views * ad.viewPrice, 0);
  const conversionRevenue = ads.reduce((sum, ad) => sum + ad.conversions * ad.conversionPrice, 0);

  const revenueBySource = [
    { id: 0, value: clickRevenue, label: 'Click Revenue', color: '#3B82F6' },
    { id: 1, value: viewRevenue, label: 'View Revenue', color: '#0D9488' },
    { id: 2, value: conversionRevenue, label: 'Conversion Revenue', color: '#10B981' },
  ];
  ...
  const dateLabels = Array(30).fill().map((_, i) => {
    const d = new Date();
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} md={8}>
          <Paper sx={{ p: 3, height: '100%' }}>
            <Typography variant="h6" sx={{ mb: 2 }}>Revenue by Source Over Time</Typography>
            <LineChart
              series={[
                {
        ...
        <Grid item xs={12} md={4}>
          <Paper sx={{ p: 3, height: '100%' }}>
            <Typography variant="h6" sx={{ mb: 2 }}>Revenue Distribution</Typography>
