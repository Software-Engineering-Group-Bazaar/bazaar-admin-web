...
const CreateRouteModal = ({ open, onClose, onCreateRoute }) => {
  ...
  const filteredOrders = mockOrders.filter(order =>
    `${order.id} ${order.senderAddress} ${order.receiverAddress}`
      .toLowerCase()
    ...
    <Modal open={open} onClose={onClose}>
      <Box sx={{
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
          ...
          <Typography variant="h6" fontWeight="bold">Create Route</Typography>
        </Box>
            ...
            {filteredOrders.map(order => (
              ...
              >
                {selectedOrders.some(o => o.id === order.id) ? (
                  <CheckSquare size={18} color="#4a0404" sx={{ mr: 3 }} />
                ) : (
                  <Square size={18} color="#ccc" sx={{ mr: 3  }} />
                )}
                <Box>
          ...
          {selectedOrders.length === 0 ? (
            ...
            selectedOrders.map(order => (
              ...
              >
                <CheckSquare size={18} color="#4a0404" sx={{ mr: 3}} />
                <Box>
                  <Typography fontWeight="medium" paddingRight="5px">Order #{order.id}</Typography>
                  <Typography variant="body2" color="text.secondary">
                    {order.senderAddress} → {order.receiverAddress}
                  </Typography>
                </Box>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 1 }}>
          ...
          <Button 
            variant="contained" 
            onClick={() => onCreateRoute(selectedOrders)}
            disabled={selectedOrders.length === 0}
            sx={{ backgroundColor: '#4a0404', '&:hover': { backgroundColor: '#3a0202' } }}
          >
            Create Route
          </Button>
        </Box>
